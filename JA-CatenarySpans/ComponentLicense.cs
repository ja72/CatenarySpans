    ...
    public class ComponentLicense : License
    {
        ...
        */
        ...
        private bool VerifyKey()
        {
            ...
            //So a developer key MUST be 53 characters long

            if (_licKey.Length != 53 && _licKey.Length != 45)
                return false;
            ...
            if (splitKey[5].Length==16 && Guid.TryParse(baseKey, out Guid guid))
            {
                string guidStr = guid.ToString();
                
                //byte[] guidBytes = guid.ToByteArray();
                var sha1 = (guidStr + Environment.UserName).HashSha1();
                //Now let's compare the SHA1 to make sure its a valid key
                if (string.Equals(splitKey[5], sha1.ToString("x8"), StringComparison.OrdinalIgnoreCase))
                {
                    return true;
                }
            }
